...
- **dst_ip 0.0.0.0 on Cilium-managed connections** — `skc_daddr` reads 0 when Cilium drops the packet before destination resolution (policy denial, conntrack lookup failure) or during loopback SNAT where the address is temporarily 0.0.0.0. Not fixable from jälki — requires Cilium debug monitor logs to diagnose the specific cause.
...
- **tcp_sock offsets hardcoded** — bytes_sent (1608) and bytes_received (1808) offsets verified on kernel 6.19.9. Other kernels may differ. TODO: pass via BPF map from BTF.
- **Runtime codegen** — any kernel function in BTF can be probed via codegen. Pre-compiled probes (tcp_connect, tcp_close, tcp_retransmit_skb) use the fast path.
